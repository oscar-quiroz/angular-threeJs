...
export class ModeloComponent implements OnInit, AfterViewInit {
  @ViewChild('canvas')
  ...
  private startRenderingLoop() {
    ...
    let component: ModeloComponent = this;
    (function render() {
  ...
  updateScreenPosition() {
    ...
    vector.x = Math.round(
      (0.5 + vector.x / 2) * (this.canvas.width / window.devicePixelRatio)
    );
    vector.y = Math.round(
      (0.5 - vector.y / 2) * (this.canvas.height / window.devicePixelRatio)
    );
